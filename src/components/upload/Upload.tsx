					...
					>
						...
						>
							<div className="dark:bg-gray-900 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
									...
									>
										...
										<div>
													...
													<div>
														<div className="flex justify-center items-center w-full">
															<label htmlFor="dropzone-file" className="flex flex-col justify-center items-center w-full h-64 bg-gray-50 rounded-lg border-2 border-gray-300 border-dashed cursor-pointer dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
																...
																<input id="dropzone-file" type="file" className="hidden" onChange={(e)=>{setFile(e.target.files!);console.log(e.target.files)}}/>
															</label>
														</div>
													</div>
													: 
											...
											<div className="flex items-center pl-4 rounded border border-gray-200 dark:border-gray-700">
												<input onClick={() => { setIsFile(!isFile); setShowAlert(false); setShowError(false); }} type="checkbox" className="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"></input>
								...
								>
									<button onClick={() => { uploadResourse(name, summary, file, url, isFile, setShowAlert, setShowError)}}
										type="button"
								...
								{showError ? <ErrorAlert title={'Error'} details={'Ha ocurrido un error inesperado'} setShowAlert={setShowError}></ErrorAlert> : <div></div>}
							</div>
						</div>
					</div>
				</div>
...
function closeModal(showUpload: any) {
	showUpload.setShowUpload(false)
}

function uploadResourse(name: string, summary: string, file: any, url: any, isFile: any, setShowAlert: any, setShowError: any) {
	if (isFile) {
		uploadFile(file, setShowAlert, setShowError);
	} else {
		console.log(name, summary, file, url)
		uploadText(name, summary, file, url, setShowAlert, setShowError);
	}
}

async function uploadText(name: string, summary: string, file: string, url: string, setShowAlert: any, setShowError: any) {
	setShowAlert(false);
	setShowError(false);
...
async function uploadFile(file: any, setShowAlert: any, setShowError: any) {
	setShowAlert(false);
	setShowError(false);
	let errors:boolean=false;
	var formData = new FormData();
	for(let result of file){
		formData.append("file", result);
		console.log(result);
		await fetch('http://localhost:3000/api/repo/uploadfile', {
		method: 'POST',
		credentials: 'include',
		body: formData
	}).then(async response => {
